...
@Setter
@ConfigReloadable
@ConfigFile("commandflow.{ext}")
@Comment("CommandFlow configuration")
public class CommandFlowConfig {
    ...
    private static List<AliasEntry> defaults() {
        ...
        list.add(new AliasEntry("/spawn", List.of("spawn"), "commandflow.alias.spawn"));
        list.add(new AliasEntry("/server hub", List.of("hub", "lobby"), "commandflow.alias.hub"));
        return list;
    }
    ...
    private static List<AliasTemplateEntry> templateDefaults() {
        ...
        entry.setPermission("commandflow.alias.agive");
